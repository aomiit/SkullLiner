...
bool pCurveOnPlane::RayCastCCW(float angleStep)
{
	...
	for (float angle=0; angle<PI*2; angle+=angleStep) 
	{
		...
		if (least>0) // Beware of division by zero
		{
			vec_x /= least;
			vec_y /= least;
		}
		...
		{
			vec_y /= abs(vec_y); // Normalize a vector
		}
		else if (vec_y == 0)
		{
			vec_x /= abs(vec_x); // Normalize a vector
		}
		...
		while (p.x>0 && p.x<size.width && p.y>0 && p.y<size.height)
		{
			...
			else if (value==0 && ray.boundInner>0 && ray.boundOuter==0)
			{
...
vector<float> __fastcall pCurveOnPlane::ApproximateParabolaInRange(float startAngle, float stopAngle, std::string wndCaption, int &numPoints)
{
	...
	// TRIAL:if (listPointsInnerBound.size()<25 || listPointsOuterBound.size()<25)
...
vector<CvPoint2D32f> __fastcall pCurveOnPlane::CollectObjectsInConstraint(vector<float> parabolaConstraints, CvRect &region )
{
	...
	for (int i=0; i<upperBound.size(); i++)
	{
		listOutput.push_back(
			cvPoint( 0.5*(upperBound[i].x + lowerBound[i].x), 0.5*(upperBound[i].y + lowerBound[i].y))
			);
	}

	// Now fit the thinned skeleton with a simple polynomial
	float* polyLine = pLeastSquareFit::fitPolynomial( listOutput, 3 );
	...
	if (this->isDebugTraceEnabled)
	{
		...
		for (int i=0; i<upperBound.size(); i++)
		{
			...
			/* DEBUG:
			pTrace->DrawCircle(
				cvPoint( upperBound[i].x*ratioX, upperBound[i].y*ratioY ),
				4,
				CV_RGB(165,250,0),
				true,
				2
			...
			pTrace->DrawCircle(
				cvPoint( lowerBound[i].x*ratioX, lowerBound[i].y*ratioY ),
				4,
				CV_RGB(75,180,255),
				true,
				2
